...
var runProgram = function runProgram(selection) {
  if (over === true) {
    store.game.over = true;
    return;
  }
  for (var i = 0; i < 9; i++) {
    var allSelection = xGameBoard.concat(oGameBoard);
...
// const gameBoardObj = ['', '', '', '', '', '', '', '', '']
var xGameBoard = [];
var xSelection = function xSelection(selection) {
  for (var j = 0; j < 9; j++) {
...
var oGameBoard = [];
var oSelection = function oSelection(selection) {
  for (var k = 0; k < 9; k++) {
...
var addX = function addX(i) {
  $('#cell-' + i).html('X');
};

var addO = function addO(i) {
  $('#cell-' + i).html('O');
};

var endGameX = function endGameX(boardChoice, turn) {
...
var endGameO = function endGameO(boardChoice, turn) {
  if (boardChoice.indexOf(0) !== -1 && boardChoice.indexOf(1) !== -1 && boardChoice.indexOf(2) !== -1) {
    // console.log(turn + ' wins')
...
module.exports = {
  ...
  oGameBoard: oGameBoard,
  gameBoardFull: gameBoardFull
};
...
var resetBoardSignOut = function resetBoardSignOut() {
  $('#cell-0').css('background-color', '#2aa2fe');
  $('#cell-1').css('background-color', '#2aa2fe');
  $('#cell-2').css('background-color', '#2aa2fe');
  $('#cell-3').css('background-color', '#2aa2fe');
...
var onShowGame = function onShowGame(event) {
  ...
  var game = data.game;
  // console.log(data)
  api.showGame(game).then(ui.onSuccess).catch(ui.onFailure);
};
...
var onUpdateGame = function onUpdateGame(event) {
  ...
  var game = data.game;
  // console.log(data)
  api.update(game).then(ui.updateSuccess).catch(ui.updateFail);
};
